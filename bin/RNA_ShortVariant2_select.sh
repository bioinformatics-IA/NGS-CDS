...
snpEff_db="${snpEff_db}/snpEff.jar"
snpSift="${snpEff_db}/SnpSift.jar"
			...
			case "$vcfFile" in
				...
				*_hap.vcf.gz)
				...
				# Skip if joint haplotype file exists and this is not the joint file
					if [ -s "$joint_file" ] && [ "$vcfFile" != "$joint_file" ]; then
...
#############################################################################################################################
	# ALL: STEP 9.2: Select Variant: Select a subset of variants from a VCF file
	#############################################################################################################################
			...
			case "$vcfFile" in
				*_MVFB.vcf)
						...
						if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|SelectVariant_M"; then
							...
							selectVariant_cmd="./gatk --java-options "-Xmx4G"  SelectVariants -R "${REFERENCE}" -V "${vcfFile}" -O "${OUTPUT_DIR}""${BAMFILE_NAME}_mselvar.vcf" --exclude-filtered"
				...
				*_HVFB.vcf)
					...
					if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|SelectVariant_H"; then
						...
						selectVariant_cmd="./gatk --java-options \"-Xms4G -Xmx4G\"  SelectVariants -R "${REFERENCE}" -V "${vcfFile}" -O "${OUTPUT_DIR}${BAMFILE_NAME}_hselvar.vcf" --exclude-filtered"
	...
	BAMFILE_NAME=$(echo "$filename" | sed -E 's/(_mselvar|_hselvar)\.vcf$//')
		...
		if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|snpEff_M"; then
				...
				echo "$BAMFILE_NAME: snpEff_M already done. Skipping snpEff..."
					...
					java -Xmx8g -jar "$snpEff" "$snpEff_db" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annM.vcf"
					update_pipeline_status "$BAMFILE_NAME" "snpEff_M"
				
		fi
			elif [ "$vcfFile" == *_hselvar.vcf ]; then
			if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|snpEff_H"; then
				...
				echo "$BAMFILE_NAME: snpEff_H already done. Skipping snpEff..."
					...
					java -Xmx8g -jar "$snpEff" "$snpEff_db" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annH.vcf"
	...
	vcfFiles="$OUTPUT_DIR*.annM.vcf $OUTPUT_DIR*.annH.vcf"	#WHY?
