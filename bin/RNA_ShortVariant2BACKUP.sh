...
variantFilterationOp="${20}" #ALL,BA,B
selectVariantStr="${21}"
...
if [ "$variantFilterationOp" = "ALL" ]; then
	#############################################################################################################################
	# ALL: STEP 9.1: Select Variant: Select a subset of variants from a VCF file
	#############################################################################################################################
	#  germline ( _hap.vcf.gz ---> _hselvar.vcf )
	#  somatic  ( _mut.vcf.gz ---> _mselvar.vcf )

	echo "Running SelectVariants..."

	#Removing starting and ending double qoutes
	selectVariantStr=$(echo "$selectVariantStr" | sed 's/^"//; s/"$//')
	joint_file="${OUTPUT_DIR}joint_hap.vcf.gz"
			...
			case "$vcfFile" in
				*_mut.vcf.gz)
						...
						if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|SelectVariant_M"; then
							...
							selectVariant_cmd="./gatk --java-options "-Xmx4G"  SelectVariants -R "${REFERENCE}" -V "${vcfFile}" -O "${OUTPUT_DIR}""${BAMFILE_NAME}_mselvar.vcf"  --restrict-alleles-to BIALLELIC"
				...
				*_hap.vcf.gz)
					...
					if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|SelectVariant_H"; then
						...
						selectVariant_cmd="./gatk --java-options \"-Xmx4G\"  SelectVariants -R "${REFERENCE}" -V "${vcfFile}" -O "${OUTPUT_DIR}${BAMFILE_NAME}_hselvar.vcf"  --restrict-alleles-to BIALLELIC"
	...
	############################################################################################
	#ALL:STEP 9.2: VariantFiltration BEFORE: Filter variant calls based on INFO and/or FORMAT annotations
	############################################################################################
	#############################################################################################
	# Germline _hselvar.vcf --> _HVFB.vcf
	# Mutect2  _mselvar.vcf --> _MVFB.vcf
			...
			case "$vcfFile" in
				*_mselvar.vcf)
					if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|VariantFiltration_M"; then
						...
						#Loop to read FilterExpression & FilterName
	...
	if [ "$annotationOPtion" = "snpeff" ]; then
		...
		BAMFILE_NAME=$(echo "$filename" | sed -E 's/(_MVFB|_HVFB)\.vcf$//')
			...
			if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|snpEff_M"; then
					...
					echo "$BAMFILE_NAME: snpEff_M already done. Skipping snpEff..."
						...
						java -Xmx8g -jar "$snpEff_path" "$snpEff_db" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annM.vcf"
						if [ $? -ne 0 ]; then
				...
				if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|snpEff_H"; then
					...
					echo "$BAMFILE_NAME: snpEff_H already done. Skipping snpEff..."
						...
						java -Xmx8g -jar "$snpEff_path" "$snpEff_db" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annH.vcf"
						if [ $? -ne 0 ]; then
							echo "Error: snpEff failed for $BAMFILE_NAME"
							exit 1
						else
		...
		vcfFiles="$OUTPUT_DIR*.annM.vcf $OUTPUT_DIR*.annH.vcf"
			...
			*_MVFB.vcf)
			...
			if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|VEP_M"; then
				...
				vep_cmd="perl vep --input_file \"${vcfFile}\" --output_file \"${OUTPUT_DIR}${BAMFILE_NAME}_vep_annM.vcf\" \
					--cache --dir_cache \"$VEP_PATH/cache\" --fasta \"$REFERENCE\" --vcf --offline \
					--assembly $vepAssembly --species $vepSpecies --sift b --polyphen b \
					--canonical --symbol --af --af_gnomad --af_1kg \
					--gene_phenotype --regulatory --numbers --biotype"
	...
	#if java -Xmx8g -jar $snpEff_path databases -v | grep -wq "$snpEff_db"; then
	if [ -f "$(dirname "$snpEff_path")/data/$snpEff_db/snpEffectPredictor.bin" ]; then
   ...
   exit 1
	...
	#VARIANTFILTERATION  after annotation
	############################################################################################
	#STEP 10: VariantFiltration: Filter variant calls based on INFO and/or FORMAT annotations
	############################################################################################
	#############################################################################################
	# Germline _hselvar.vcf --> _HVF.vcf
	# Mutect2  _mselvar.vcf --> _MVF.vcf

	echo "Running VariantFilteration after annotation..."
	...
	case "$filename" in
			...
			java -Xmx8g -jar "$snpSift" filter "$filterOptionsA" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}_vep_annM.sift.vcf"
			...
			echo "Annotating SOMATIC variant (EXON): $filename"
			java -Xmx8g -jar "$snpSift" filter "$filterOptionsA" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}_vep_annH.sift.vcf"
			...
			echo "Annotating SOMATIC variant (EXON): $filename"
			java -Xmx8g -jar "$snpSift" filter "$filterOptionsA" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annM.sift.vcf"
			...
			echo "Annotating SOMATIC variant (EXON): $filename"
			java -Xmx8g -jar "$snpSift" filter "$filterOptionsA" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annH.sift.vcf"
	...
	echo "Running VariantFiltration Before Annotation..."
	...
	case "$vcfFile" in
		*_mut.vcf.gz)
							if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|VariantFiltration"; then
				# If both sample and stage exist in the file
				echo "$BAMFILE_NAME: VariantFiltration_M already done. Skipping VariantFiltration..."
			else
			if [ "$germSomaticOP" = "SOMATIC" ] || [ "$germSomaticOP" = "BOTH" ]; then
			filterVariant_cmd="./gatk --java-options "-Xmx4G"  VariantFiltration -R "${REFERENCE}" -V "${vcfFile}" -O "${OUTPUT_DIR}""${BAMFILE_NAME}_MVFB.vcf""
		#Loop to read FilterExpression & FilterName
		if [ -z "$filterOptionsB" ]; then
	...
	update_pipeline_status "$BAMFILE_NAME" "VariantFiltration_M"
		...
		*_hap.vcf.gz)
		...
		filterVariant_cmd="./gatk --java-options "-Xmx4G"  VariantFiltration -R "${REFERENCE}" -V "${vcfFile}" -O "${OUTPUT_DIR}""${BAMFILE_NAME}_HVFB.vcf""
	...
	if java -Xmx8g -jar $snpEff databases -v | grep -wq "$snpEff_db"; then
		echo "$snpEff_db is already installed"
	else
		# If the database is not installed, download it
		echo "Installing $snpEff_db ..."
		
		java -Xmx8g -jar $snpEff download -v "$snpEff_db"
...
elif [ "$variantFilterationOp" = "B" ]; then
	...
	echo "Running VariantFilteration Before Annotation..."
	...
	case "$vcfFile" in
		*_mut.vcf.gz)
		...
		echo "VariantFiltration Command is : $filterVariant_cmd"
		eval $filterVariant_cmd
	update_pipeline_status "$BAMFILE_NAME" "VariantFiltration"
			fi
			fi
			;;
		*_hap.vcf.gz)
	...
	BAMFILE_NAME=$(echo "$filename" | sed -E 's/(_MVFB|_HVFB)\.vcf$//')
			...
			if grep -q "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" && grep "^${BAMFILE_NAME}|" "$PIPELINE_STATUS_FILE" | grep -q "|snpEff_H"; then
				...
				echo "$BAMFILE_NAME: snpEff_H already done. Skipping snpEff..."
					...
					echo "Annotating GERMLINE variant: $filename"
					java -Xmx8g -jar "$snpEff_path" "$snpEff_db" "$vcfFile" > "${OUTPUT_DIR}${BAMFILE_NAME}.annH.vcf"
					update_pipeline_status "$BAMFILE_NAME" "snpEff"
				fi
			fi
	fi
	done
